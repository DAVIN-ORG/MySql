select * from (select product,day,revenue, rank() over (order by revenue desc) as ranking from groceries) t where t.ranking=3;

create external table if not exists movies] ( movie_id int, movie_name string)
comment 'movie_table'
row format delimited 
fields terminated by '::'
stored as textfile
location 'C:\Users\dev30\datasets\movies.dat';
